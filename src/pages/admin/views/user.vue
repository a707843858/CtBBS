...
export default {
    ...
    created(){
        //获取user全部信息
        var self = this ;
        this.get_user({start:0,limit:10}).then(res=>{this.userData = res.data});
        this.get_count({table:'wp_user',meta:'id'}).then(res=>{this.value.page_total = parseInt(res.data[0]['COUNT("id")'])});
        //获取总条数，用于分页
